# Auto-Prime-Veiculos
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>AutoPrime Veículos</title>
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&display=swap" rel="stylesheet">
    <style>
        /* RESET E FONTE */
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Roboto', sans-serif; }
        body { background-color: #111; color: #fff; }

        /* BANNER PRINCIPAL */
        header {
            background: linear-gradient(90deg, #ff0000, #ff9900);
            padding: 60px 20px;
            text-align: center;
        }
        header h1 {
            font-size: 3rem;
            margin-bottom: 10px;
            text-shadow: 2px 2px #000;
        }
        header p {
            font-size: 1.5rem;
            font-weight: bold;
            color: #fff;
        }

        /* BIO */
        .bio {
            background-color: #222;
            padding: 30px 20px;
            text-align: center;
            font-size: 1.2rem;
            line-height: 1.6;
        }

        /* SEÇÃO DE CARROS */
        .cars {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));
            gap: 20px;
            padding: 40px 20px;
        }
        .car-card {
            background-color: #1a1a1a;
            border: 2px solid #ff0000;
            border-radius: 10px;
            overflow: hidden;
            text-align: center;
            transition: transform 0.3s;
        }
        .car-card:hover { transform: scale(1.05); }
        .car-card img { width: 100%; height: 180px; object-fit: cover; }
        .car-card h3 { margin: 15px 0 5px; color: #ffcc00; }
        .car-card p { margin-bottom: 15px; font-weight: bold; color: #fff; }
        .car-card button {
            background-color: #ff0000;
            border: none;
            padding: 10px 20px;
            margin-bottom: 15px;
            color: #fff;
            font-weight: bold;
            cursor: pointer;
            border-radius: 5px;
            transition: background-color 0.3s;
        }
        .car-card button:hover { background-color: #cc0000; }

        /* VANTAGENS */
        .advantages {
            background-color: #222;
            padding: 40px 20px;
            text-align: center;
        }
        .advantages h2 { margin-bottom: 20px; color: #ffcc00; }
        .advantages ul { list-style: none; }
        .advantages li {
            margin: 10px 0;
            font-size: 1.2rem;
            position: relative;
        }
        .advantages li::before {
            content: "✔️";
            position: absolute;
            left: -30px;
        }

        /* DEPOIMENTOS */
        .testimonials {
            background-color: #111;
            padding: 40px 20px;
            text-align: center;
        }
        .testimonials h2 { color: #ffcc00; margin-bottom: 20px; }
        .testimonial {
            margin-bottom: 20px;
            font-style: italic;
        }

        /* CONTATO */
        .contact {
            background: linear-gradient(90deg, #ff0000, #ff9900);
            padding: 40px 20px;
            text-align: center;
        }
        .contact h2 { margin-bottom: 20px; color: #fff; }
        .contact p { margin: 10px 0; font-weight: bold; font-size: 1.2rem; }
        .whatsapp-button {
            display: inline-block;
            margin-top: 15px;
            background-color: #25D366;
            color: #fff;
            font-weight: bold;
            padding: 15px 25px;
            border-radius: 10px;
            text-decoration: none;
            animation: pulse 1.5s infinite;
        }
        @keyframes pulse {
            0% { transform: scale(1); }
            50% { transform: scale(1.1); }
            100% { transform: scale(1); }
        }

        /* RODAPÉ */
        footer {
            background-color: #111;
            padding: 20px;
            text-align: center;
            color: #fff;
            font-size: 0.9rem;
        }
    </style>
</head>
<body>

    <!-- BANNER PRINCIPAL -->
    <header>
        <h1>AutoPrime Veículos</h1>
        <p>🔥 OFERTAS IMPERDÍVEIS POR TEMPO LIMITADO – GARANTA SEU CARRO HOJE!</p>
    </header>

    <!-- BIO -->
    <div class="bio">
        Na AutoPrime Veículos você encontra carros novos e seminovos com preços que cabem no seu bolso! 
        Qualidade garantida, financiamento facilitado e atendimento rápido. Mais de 500 clientes satisfeitos! 
        Não perca nossas promoções exclusivas por tempo limitado!
    </div>

    <!-- SEÇÃO DE CARROS -->
    <section class="cars">
        <div class="car-card">
            <img src="https://www.chevrolet.com.br/content/dam/chevrolet/mercosul/brazil/vehicles/2024/onix/mov/01-images/onix-2024.jpg" alt="Chevrolet Onix 2024">
            <h3>Chevrolet Onix 2024</h3>
            <p>R$ 79.990</p>
            <button>COMPRAR AGORA</button>
        </div>
        <div class="car-card">
            <img src="https://www.hyundai.com.br/on/demandware.static/-/Sites-hyundai-master-catalog/default/dw8f0c60c7/images/2023/hb20.png" alt="HB20 2023">
            <h3>HB20 2023</h3>
            <p>R$ 72.500</p>
            <button>COMPRAR AGORA</button>
        </div>
        <div class="car-card">
            <img src="https://www.toyota.com.br/wp-content/uploads/2022/06/Corolla-2022.jpg" alt="Toyota Corolla 2022">
            <h3>Toyota Corolla 2022</h3>
            <p>R$ 109.900</p>
            <button>COMPRAR AGORA</button>
        </div>
        <div class="car-card">
            <img src="https://www.fiat.com.br/content/dam/fiat-br/vehicles/argo/2023/argo-2023.png" alt="Fiat Argo 2023">
            <h3>Fiat Argo 2023</h3>
            <p>R$ 68.900</p>
            <button>COMPRAR AGORA</button>
        </div>
        <div class="car-card">
            <img src="https://www.jeep.com.br/content/dam/jeep/vehicles/renegade/2021/renegade-2021.png" alt="Jeep Renegade 2021">
            <h3>Jeep Renegade 2021</h3>
            <p>R$ 98.500</p>
            <button>COMPRAR AGORA</button>
        </div>
        <div class="car-card">
            <img src="https://www.vw.com.br/content/dam/volkswagen/vehicles/polo/2024/polo-2024.png" alt="Volkswagen Polo 2024">
            <h3>Volkswagen Polo 2024</h3>
            <p>R$ 82.000</p>
            <button>COMPRAR AGORA</button>
        </div>
        <div class="car-card">
            <img src="https://www.honda.com.br/civic-2020.png" alt="Honda Civic 2020">
            <h3>Honda Civic 2020</h3>
            <p>R$ 95.000</p>
            <button>COMPRAR AGORA</button>
        </div>
    </section>

    <!-- VANTAGENS -->
    <section class="advantages">
        <h2>Por que comprar conosco?</h2>
        <ul>
            <li>Financiamento facilitado em até 60x</li>
            <li>Entrada parcelada</li>
            <li>Avaliamos seu carro na troca</li>
            <li>Garantia de procedência</li>
        </ul>
    </section>

    <!-- DEPOIMENTOS -->
    <section class="testimonials">
        <h2>Depoimentos de Clientes</h2>
        <p class="testimonial">"Comprei meu carro aqui e foi tudo perfeito! Atendimento top e preço justo." – Maria S.</p>
        <p class="testimonial">"Financiei meu Onix 2024 sem burocracia. Super recomendo!" – João P.</p>
    </section>

    <!-- CONTATO -->
    <section class="contact">
        <h2>Entre em Contato</h2>
        <p>📞 WhatsApp: <a href="https://wa.me/5511953425162" class="whatsapp-button">11953425162</a></p>
        <p>📸 Instagram: <a href="https://www.instagram.com/ferrsyx" target="_blank">@ferrsyx</a></p>
        <p>📍 Endereço: Avenida São Miguel Paulista, São Paulo – SP</p>
    </section>

    <!-- RODAPÉ -->
    <footer>
        AutoPrime Veículos – Realizando sonhos sobre rodas 🚗✨
    </footer>

</body>
</html>